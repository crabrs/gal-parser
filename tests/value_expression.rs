...
fn case_expression() {
    parse_assert!(
        ...
        "CASE n WHEN IS NULL THEN 1 ELSE 0 END"
    );
    parse_assert!(Rule::case_expression, "CASE WHEN a > b THEN 1 ELSE 0 END");
    parse_assert!(
        Rule::case_expression,
        "CASE p WHEN IS NULL THEN -1 ELSE path_length(p) END"
    );
}
...
fn common_value_expression() {
    parse_assert!(
        ...
        "DURATION_BETWEEN(n.creationDate, $creationDate)"
    );
    parse_assert!(Rule::common_value_expression, "-1");
}

#[test]
fn duration_value_expression() {
    parse_assert!(
        Rule::duration_value_expression,
        "DURATION_BETWEEN(n.creationDate, $creationDate)"
    );
}
...
fn boolean_value_expression() {
    parse_assert!(
        ...
        "DURATION_BETWEEN(n.creationDate, $creationDate) > 10"
    );
}
...
fn datetime_value_expression() {
    parse_assert!(
        Rule::local_datetime_function,
        "local_datetime({epochMillis: foaf.birthday})"
    );
}
